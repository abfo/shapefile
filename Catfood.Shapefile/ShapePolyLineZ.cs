...
{
    /// <summary>A Shapefile PolyLineZ shape with optional measures.</summary>
    public class ShapePolyLineZ : ShapePolyLineM
    {
        /// <summary>Creates a Shapefile PolyLineZ shape from a record.</summary>
        ...
        protected internal ShapePolyLineZ(int recordNumber, StringDictionary metadata, IDataRecord dataRecord, byte[] shapeData)
            : this(recordNumber, metadata, dataRecord, ShapeRecordData.Parse(shapeData, true, true))
        {
        }

        private ShapePolyLineZ(int recordNumber, StringDictionary metadata, IDataRecord dataRecord, ShapeRecordData data)
            : base(ShapeType.PolyLineZ, recordNumber, metadata, dataRecord, data)
        {
            Zmin = data.Zmin;
            Zmax = data.Zmax;
            Z = data.Z;
